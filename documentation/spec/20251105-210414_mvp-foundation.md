...
**Last updated:** 2025-11-09 20:40 UTC
...
- **2025-11-09T20:40Z** – Follow-up: add layered stream-health guards (per-chunk amplitude, zero-length detection, track liveness) and cut the abort timer to 9 s.
- **2025-11-09T20:07Z** – Add a 15 s “no audio” safeguard that plays a soft chime and aborts the recording with the usual error path when capture never produces real audio.
- **2025-11-09T19:44Z** – Ensure mobile transcription list resizes beneath the capture controls by dynamically allocating the remaining viewport height.
...
### No-Audio Safety Stop — Delivered 2025-11-09
- Detects real audio flow using chunk amplitude metrics; if no usable audio lands within 9 s, the app plays a low-volume tone, stops recording, and surfaces the existing “no audio captured” error.
- Clears the watchdog once meaningful audio arrives, logging the measured levels for later inspection, and continuously polls track readiness/noise so muted or ended streams abort quickly.
- Flags consecutive silent or zero-length chunks (including analysis failures) to trigger the same fail-safe before users waste time on a dead microphone.

### Mobile Layout Guardrails — Delivered 2025-11-09
- Switched the narrow-screen layout to a flex column so the capture controls reserve their full height and the transcription list automatically fills the remaining viewport space without covering the record button.
