...
- 2026-03-25: **Dense load-path instrumentation** — `loadSessions` uses `stepMs` + `loadPathMs` on ~20 milestones; `listSessions` splits **getAll** vs **map** vs **sort** (`syncMs` / `awaitMs`); `listSnipsForSessions` logs IDB vs normalize phases + optional `chunkIndex`/`chunkTotal`; `refreshTranscriptionPreviews` uses `markPv` with `stepMs`/`previewPathMs` per prune, chunk, yield; `manifest.init` logs `after getDB await`; `AGENTS.md`: no auto-commit unless user asks.
